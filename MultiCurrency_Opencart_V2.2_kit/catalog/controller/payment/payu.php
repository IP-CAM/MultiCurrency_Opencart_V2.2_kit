...
class ControllerPaymentPayu extends Controller {
	...
	public function index() {	
		...
		if($this->config->get('payu_test')=='demo')
		{
			...
			$txnid        = 	$this->session->data['order_id'] + 263737739;
		}
		else
		{
        ...
        //$data['surl'] = 'https://ayush.com/opencart2.1/index.php?route=payment/payu/callback';
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/payu.tpl')) {
			...
			return $this->load->view('payment/payu.tpl', $data);
		}		
	...
	public function callback() {
		...
		if (isset($this->request->post['key']) && ($this->request->post['key'] == isset($key))) {
			...
			$data['text_failure_wait'] = sprintf($this->language->get('text_failure_wait'), $this->url->link('checkout/cart'));
			 ...
			 if($this->config->get('payu_test')=='demo')
				{
					$orderid = $this->request->post['txnid']- 263737739;
				} else 
				{
